---
title: "Part 1-Gene Expression Analysis(DESeq2)"
output: 
  html_document:
    theme: cerulean
    toc: true
...
This analysis is based on the dataset **GSE171663** from a study by [Uryga et al. (2021)](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE171663). The authors investigated transcriptomic changes in **Vascular Smooth Muscle Cells (VSMCs)** undergoing **Replicative Senescence**.

The workflow used here was adapted from the **Sanbomics** tutorial series and standard RNA-seq best practices. In this section, we will:

1.  **Perform Quality Control (QC):** Assess sample-level variance using PCA and hierarchical clustering.
2.  **Differential Expression Analysis:** Use `DESeq2` to identify genes changing between Proliferating (Control) and Senescent (RS) conditions.
3.  **Visualization:** Generate high-quality visualizations including Volcano plots and Heatmaps to validate the biological phenotype.

## 0. Prerequisites - Alignment and Read counts generation in bash
...
Counts <- read.delim(paste0(in_path, "counts.csv"), header = TRUE, row.names = 1, sep = ",")
...
## 4a. Quality Control plots for Normalization, Variance Stabilization, Dimensionality Reduction
```{r}
...
#(norm_counts, file = paste0(results, "norm_counts.csv")) #Save normalized counts file
...
cor_h <- Heatmap(vsd_cor_data, name = "r coeff.", col = colorRampPalette(my_colors)(256))
...
# write.csv(res_all, file = paste0(results,"DEgenes-all_ensembl.csv"))
...
res_all #preview the data
```

```{r}
# count for any duplicate values then sort by highest count. Remove NAs
r_count <- res_all %>% dplyr::count(symbol, sort = TRUE) #n = 500/13000 genes unannotated == NAs
res_all <- na.omit(res_all)
r_count <- res_all %>% dplyr::count(symbol, sort = TRUE)

#save the annotated file of all DGE list
# write.csv(res_all, file = paste0(results,"DEgenes-all_symbol.csv"))
...
# write.csv(sig_res, file = paste0(results,"DEgenes-sig_ens.csv"))

#Annotate significant genes with gene symbol
sig_res$symbol <- mapIds(edb, keys = rownames(sig_res), keytype = "GENEID", column = "SYMBOL")

# count for any duplicate values then sort by highest count. Remove NAs
s_count <- sig_res %>% dplyr::count(symbol, sort = TRUE) #1 unmapped NA
sig_res <- na.omit(sig_res)
s_count <- sig_res %>% dplyr::count(symbol, sort = TRUE)

sig_res
#save the significant DE gene with symbol 
# write.csv(sig_res, file = paste0(results,"DEgenes-sig.csv"))
...
#pdf(file = paste0(figures, "DGE_volcano.pdf"), width = 7, height = 3.5)
print(e)
#dev.off()

# ggsave(
#   filename = paste0(figures,"volcano_plot.png"),
#   plot = e,
#   width = 7,
#   height = 3.5,
...
```{r heatmap-plot, fig.width=6, fig.height=12}
...
h<- Heatmap(counts.z, cluster_rows = T, cluster_columns = T, column_labels = colnames(counts.z),
        name = "Z-score",heatmap_width = unit(5,'in'), heatmap_height = unit(10,'in'), row_labels = sig_res_hm[rownames(counts.z),]$symbol, )

##save as pdf
#pdf(paste0(figures,'simple_heatmap.pdf'), width = 4, height = 8) 

##save as png
# png(filename = paste0(figures,"DE_heatmap.png"),
#     width = 5,
#     height = 10,
#     units = 'in',
#     res = 300, # <--- This is the key. 300 is "Print Quality"
#     bg = "white"
# )
print(h)
# dev.off()
