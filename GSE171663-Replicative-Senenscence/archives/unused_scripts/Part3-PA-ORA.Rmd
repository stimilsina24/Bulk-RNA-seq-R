---
title: "Part 3- Overrepresentation analysis(ORA)"
output: 
  html_document:
    theme: cerulean
    toc: true
...
figures <- paste0(dirname(getwd()), "/unused_figures/ORA/") # output path, where you want your figures exported to
...
# saveRDS(ego_BP_UP, file = paste0(r_objects,"ego_BP_UP.RDS"))
# ego_BP_UP <- readRDS(file = paste0(r_objects,"ego_BP_UP.RDS"))
...
# write.csv(ego_BP_UP, file = paste0(results, "EGO_BP_UP_all.csv"))
# write.csv(BP_UP, file = paste0(results, "EGO_BP_UP_top10.csv"))

#goplot Up genes
#pdf(file = paste0(figures, "EGO_BP.pdf"), width = 10, height = 8, bg = "white")
# gpu <- goplot(ego_BP_UP, showCategory = 10)
# print(gpu)

#barplot Up genes
#pdf(file = paste0(figures, "GO_bar_BP.pdf"), width = 6, height = 6, bg = "white")
...
# Save the RDS file
#saveRDS(ego_BP_DN, file = paste0(r_objects,"ego_BP_DN.RDS"))

#Load data from RDS file
#ego_BP_DN <- readRDS(paste0(r_objects,"ego_BP_DN.RDS"))
...
# write.csv(ego_BP_DN, file = paste0(results, "EGO_BP_DN_all.csv"))
# write.csv(BP_DN, file = paste0(results, "EGO_BP_DN_top10.csv"))

# GO plot down genes
#pdf(file = paste0(figures, "EGO_BP_DN.pdf"), width = 10, height = 8, bg = "white")
# gpd <- goplot(ego_BP_DN, color = "p.adjust", title = "Biological Processes(down") 
# print(gpd)


# For barplot:
#pdf(file = paste0(figures, "EGO_bar_DN.pdf"), width = 6, height = 6, bg = "white")
...
# write.csv(BP_comb_top10, file = paste0(results, "GO_BP_top10_combined.csv"))

#import the file
# BP_comb_top10 <- read.csv(file = paste0(results,"GO_BP_top10_combined.csv")
...
#pdf(file = paste0(figures, "EGO_BP_combo_bar.pdf"), width = 8, height = 6, bg = "white")
...
#saveRDS(GBP, file = paste0(r_objects, "EGO_BP_combo_gg.RDS"))
...
# BP_comb_top10 <- read.csv(file = paste0(results,"GO_BP_top10_combined.csv"), row.names = 1)
...
#save
#write.csv(BP_comb_top10, file = paste0(results, "BP_top10_correctfinal.csv", row.names = F))

#read in the corrected file
# BP_comb_top10 <- read.csv(file = paste0(results,"BP_top10_correctfinal.csv"), row.names = 1)

BP_comb_top10$FC <- factor(BP_comb_top10$FC) 
levels(BP_comb_top10$FC) <- c("DOWN", "UP")

BP_comb_top10 <- BP_comb_top10 %>%
  arrange(-GeneRatio)

#Create ggplot dotplot
#pdf(file = paste0(figures, "EGO_BP_combo_dot.pdf"), width = 8, height = 6, bg = "white")
...
#Save the Figure object
#saveRDS(dot_BP, file = paste0(r_objects, "EGO_BP_combo_dot.RDS"))
...
# ego_BP_UP <- readRDS(paste0(r_objects,"ego_BP_UP.RDS"))
...
#write.csv(geneList_S, file = paste0(results, "Ranked_ENTREZ_GSEA_Symbol.csv"), row.names = F) #Save the ranked genelist with symbols
...
#pdf(file = paste0(figures, "EGO_BP_heatplot_combo.pdf"), width = 36, height = 14, bg = "white")
...
#pdf(file = paste0(figures, "EGO_BP_cnet_up or down.pdf"), width = 25, height = 30, bg = "white")
...
#pdf(file = paste0(figures, "EGO_BP_emap_up or down.pdf"), width = 16, height = 7, bg = "white")
...
# pdf(file = paste0(figures, "EGO_BP_tree_up or down_A.pdf"), width = 22, height = 18, bg = "white")
# tpa <- aplot::plot_list(pt2, pt1, tag_levels='a')
# print(tpa)
# 

#pdf(file = paste0(figures, "EGO_BP_tree_up or down.pdf"), width = 16, height = 18, bg = "white")
